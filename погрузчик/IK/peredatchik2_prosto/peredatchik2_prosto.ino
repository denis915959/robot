...
void ReadData()
{
  ...
  radio.openReadingPipe(1, 0xF0F0F0F066); // Какой канал считывать, 40-разрядный адрес
  radio.startListening();                 // Остановить передачу и начать прием
...
led_array[1].num_led=1;
led_array[1].num_sig=2;// ПОТОМ УБРАТЬ!, было 3

led_array[2].num_led=1;
led_array[2].num_sig=6;

led_array[3].num_led=2;//5 - поворот вправо, 7 - дублирующий
led_array[3].num_sig=4;//если 3 или 4, то нужно 7 здесь, 7=5, 8=6; 8 и 7 только после взятия ящика, это аналог СТАРЫХ 11 и 12 по сути, было 8
//3-поворот вправо и взять ящик снизу
led_array[4].num_led=2;//5 - поворот вправо, 7 - дублирующий
led_array[4].num_sig=6;
led_array[5].num_led=3;//в дальнейшем, возможно, можно будет убрать
led_array[5].num_sig=5;

led_array[6].num_led=0;//в дальнейшем, возможно, можно будет убрать
led_array[6].num_sig=0;
