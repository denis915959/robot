...
led[2].num_led=2;
led[0].num_sig=6;
led[1].num_sig=2;//3-поворот вправа и взять ящик снизу
led[2].num_sig=5;//
led[0].rotate=10;
led[1].rotate=11;//11
led[2].rotate=10;
...
//VS.send(0xFDD54E);//если будет тормозить, то в этот switch закинем весь цикл приема сигнала с погрузчика
  ...
  case 5:
    //if( mySwitch.available() ){//прием сигнала с погрузчика
        ...
        if (value<10){
        Serial.println(value);  
        if (value==5)//возможное место проблем, в тслучае чего добавить mySwitch2.send(10, 5);if (value==7). если что, было 7
          c=0;         
        }
        //mySwitch.resetAvailable();
   // }//конец приема сигнала с погрузчика
