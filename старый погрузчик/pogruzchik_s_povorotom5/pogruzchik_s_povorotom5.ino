...
int t=1000;//время поворота без датчика, 1500             ЕСЛИ ЧТО, ПРОБЛЕМА С ПОВОРОТОМ МОЖЕТ БЫТЬ ЗДЕСЬ
int N_z=120;//230
int st=120;//170
int sklad=120;//100
...
void rotate_left_stel()
{
  ...
  analogWrite(11, n);
  delay(t);//конец поворота по таймеру    
  while (digitalRead(3)!=1)//поворот по датчику
  {
    digitalWrite(7, HIGH);
    digitalWrite(8, LOW);
    analogWrite(6, n);
...
/*void rotate_left_180()
{
  ...
  digitalWrite(10, LOW);
  analogWrite(11, n);
  delay(t);//конец поворота по таймеру, в случае чего изменить на поворот на 90 + сразу поворот на 90 
  while (digitalRead(23)!=1)//поворот по датчику
  {
    digitalWrite(7, HIGH);
    digitalWrite(8, LOW);
...
//  servo1.write(rotate_v);
  ...
  delay(t_operezhenie+t);//конец доворота по таймеру
  while (digitalRead(19)!=1)//поворот по датчику, было 23
  {
    digitalWrite(8, HIGH);
    digitalWrite(7, LOW);
    analogWrite(6, n);
...
void rotate_right_stel()//поворот вправо
{
  ...
  delay(t);//конец поворота по таймеру
//  servo1.write(rotate_v);
  while (digitalRead(19)!=1)//поворот по датчику
  {
    digitalWrite(8, HIGH);
    digitalWrite(7, LOW);
    analogWrite(6, n);
...
void go_front_to_stellazh()
{
  ...
  /*while (distance > 7)//отрубили датчик на платформе и исключили ИК датчик на шасси, если будет врезаться - сделать расстояние 8
    {
      //goto lin;
     // distance = pered.read();
      servo1.write(rotate_v);
    ...
    distance = pered.read();
Serial.println(distance);
while (distance>4)//(digitalRead(37)!=1)//движение назад по линии// 14 было, 10
{
digitalWrite(10, LOW);
digitalWrite(9, HIGH);
analogWrite(11, N);
...
void go_back()
{
   ...
   digitalWrite(7, LOW);
   digitalWrite(8, LOW);
   while (distance1>22)// while (digitalRead(37)!=1)//движение назад по линии,  было 23
    {  
      /*right_sensor_val = digitalRead(22);
      left_sensor_val = digitalRead(23);
      right_sensor_2_val = digitalRead(24);
...
void go_back_timer()
{
  ...
  digitalWrite(7, LOW);
  digitalWrite(8, LOW);
  while (distance1>12)//(digitalRead(37)!=1)//движение назад по линии// 14 было, 10
    {  
     ...
     digitalWrite(7, LOW);
     digitalWrite(8, LOW);*/
     distance_szadi[0] = nazad.read();
distance_szadi[1]=distance_szadi[0];
Serial.println(distance_szadi[1]);
while (distance_szadi[1]>10)//(digitalRead(37)!=1)//движение назад по линии// 14 было, 10
{
digitalWrite(9, LOW);
digitalWrite(10, HIGH);
analogWrite(11, N);
...
if (left_sensor_szadi_val==1)
  ...
  while (abs(distance_szadi[0]-distance_szadi[1])>=2)
  {
    distance_szadi[1]=distance_szadi[0];
...
bool flag=false;
bool go_line=true;
bool radio_rotate=false;//если два раза подряд поворот, то радио будет глючить. Если хотя бы одна другая команда между ними, то цифра в буфер меняется
...
while (digitalRead(23)!=1)//поворот по датчику
{
   ...
   if (radio_rotate==true)//8 после взятия ящика только
    {
      for (int i=0; i<number_povtor; i++)
      {
        data[0]=8;
...
//c=1;
    ...
    analogWrite(11, 0);
    delay(d);
    if (radio_rotate==true)//7 после взятия ящика только
    {
      for (int i=0; i<number_povtor; i++)
      {
        data[0]=7;
